        ...
        return df \
            .assign(
                ...
                region=lambda x: x.region \
                    .replace('Bedford city', 'Bedford County')
            ) \
...
def _indicators_create(df):
    ...
    return df \
        .assign(
            # tee up values: I return a nan instead of the total if all age 
            # categories are not reported.
            EmpMid=lambda x: (x['Emp'] + x['EmpEnd']) / 2,
            total_emp=lambda x: x[['EmpMid', 'fips', 'time']] \
                .groupby(['fips', 'time']) \
                .transform(lambda y: y.sum() if y.count() == 5 else np.nan),
        ) \
        .assign(
            # indicators create
        ...
        [[
            'fips', 'geo_level', 'firmage', 'time', 'contribution', 
            'compensation', 'constancy', 'creation'
        ]] \
    ...
    return df \
        ...
        .assign(
            demographic=lambda x: pd.Categorical(
                ...
                [
                    'Ages 0 to 1', 'Ages 2 to 3', 'Ages 4 to 5', 'Ages 6 to 10', 
                    'Ages 11+'
                ]
            ),
        ...
        .rename(columns={
            'time': 'year', 'type':'demographic-type', 
            'firmage':'demographic-code'
        }) \
...
def _enforce_geo_universe(df, geo_level):
    ...
    return c.geography_universe[['fips', 'geo_level', 'name']] \
        ...
        .rename(
            columns={'type':'demographic-type', 'code':'demographic-code'}
        ) \
        .merge(
            df, 
            on=[
                'fips', 'geo_level', 'demographic-type', 'year', 'demographic', 'demographic-code'
            ], 
            how='left'
        )
...
def _website_csvs_save(df, aws_filepath):
    for indicator in ['contribution', 'compensation', 'constancy', 'creation']:
        ...
        df_out.to_csv(
            c.filenamer(f'data/eji_website_{indicator}.csv'), index=False
        )
        if aws_filepath:
            df_out.to_csv(
                f'{aws_filepath}/eji_website_{indicator}.csv', index=False
            )
...
def eji_data_create_all(
    raw_data_fetch, raw_data_remove, qwi_n_threads, aws_filepath=None
):
    """
    Create and save EJI data. This is the main function of eji_command.py. 

    Fetch raw QWI, PEP, and MSA-crosswalk data, transform it, and save it to two
